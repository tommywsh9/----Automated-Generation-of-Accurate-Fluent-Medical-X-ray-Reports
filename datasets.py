...
class NLMCXR(data.Dataset): # Open-I Dataset
    def __init__(self, directory, input_size=(256,256), random_transform=True,
        ...
        self.target_sections = ['FINDINGS']
        self.vocab = spm.SentencePieceProcessor(model_file='C:/XRay/Vocabulary/' + vocab_file)
        self.vocab_file = vocab_file # Save it for subsets
    ...
    def __get_nounphrase(self, top_k=100, file_name='count_nounphrase.json'):
        count_np = json.load(open('open-i/' + file_name, 'r'))
        sorted_count_np = sorted([(k,v) for k,v in count_np.items()], key=lambda x: x[1], reverse=True)
        top_nounphrases = [k for k,v in sorted_count_np][:top_k]
        return top_nounphrases
    ...
    def __input_caption(self):
        with open('Vocabulary/captions.json') as f:
...
class FFAIR(data.Dataset):  # FFA-IR Dataset
    def __init__(self, directory, split, input_size=(256, 256), random_transform=True,
                 view_pos=['AP', 'PA', 'LATERAL'], max_views=2, sources=['image', 'history'], targets=['label'],
                 max_len=1000, vocab_file='ffair_unigram_1700.model'):
    ...
    def __len__(self):
        if(self.split == 'test'):
            return 1
        return len(self.file_list[self.split])

    def __getitem__(self, idx):
        # 加上filename等
        file_name = self.file_list[self.split][idx]
        sources, targets = [], []
        # tmp_rep = self.captions[file_name]

        # ------ Multiview Images ------
        if 'image' in self.sources:
            imgs, vpos = [], []
            images = self.file_report[file_name]['Image_path']

            # Randomly select V images from each folder
            new_orders = np.random.permutation(len(images))  #np.random.permutation() 总体来说他是一个随机排列函数,就是将输入的数据进行随机排列
            img_files = np.array(images)[new_orders].tolist()

            for i in range(min(self.max_views, len(img_files))):
                #img_file = 'E:/BaiduNetdiskDownload/ffa-ir-towards-an-explainable-and-reliable-medical-report-generation-benchmark-1.0.0/ffa-ir-towards-an-explainable-and-reliable-medical-report-generation-benchmark-1.0.0/FFAIR/' + img_files[i]
                img_file = 'C:/test.jpeg'
        ...
        #self.source_sections = ['INDICATION', 'COMPARISON']
        # self.target_sections = ['FINDINGS']
        target_info = self.file_report[file_name]['En_Report']  # 报告的英文
        target_info = self.preprocess(target_info)
        ...
        for i in range(len(self.sources)):
            if self.sources[i] == 'image':
                sources.append((imgs, vpos))   #vpos是image_mask
            if self.sources[i] == 'label':
                # print(self.file_labels)
                sources.append(np.concatenate([self.file_labels[file_name.split('/')[0]], np_labels]))
            if self.sources[i] == 'caption':
                sources.append(target_info)
            if self.sources[i] == 'caption_length':
                sources.append(min(len(encoded_target_info), self.max_len))
        ...
        for i in range(len(self.targets)):
            if self.targets[i] == 'label':
                # print(file_name)
                targets.append(np.concatenate([self.file_labels[file_name.split('/')[0]], np_labels]))
            if self.targets[i] == 'caption':
                targets.append(target_info)
            if self.targets[i] == 'caption_length':
                targets.append(min(len(encoded_target_info), self.max_len))
    ...
    def __get_nounphrase(self, top_k=100, file_name='count_nounphrase.json'):
        count_np = json.load(open('C:/XRay/data/FFAIR/' + file_name, 'r'))
        sorted_count_np = sorted([(k, v) for k, v in count_np.items()], key=lambda x: x[1], reverse=True)
        top_nounphrases = [k for k, v in sorted_count_np][:top_k]
        return top_nounphrases  #提取名词短语
    ...
    def __input_label(self):
        with open('C:/XRay/data/FFAIR/file2label.json') as f:
            labels = json.load(f)
        self.file_labels = labels

    def __input_caption(self):
        with open('C:/XRay/data/FFAIR/captions.json') as f:
            captions = json.load(f)
        self.captions = captions   #"case_0/0.jpeg": "1. neovascular dye leakage from the optic disc surface was seen during the left eye imaging; 2. reduced retinal arteriovenous branches, venous tortuous dilatation and bead-like changes were seen in the posterior pole and mid-periphery during the left eye imaging; 3. lamellar non-perfused areas were seen in the posterior pole and mid-periphery during the left eye imaging.\n",

    def __input_report(self):
        with open('C:/XRay/data/FFAIR/ffair_annotation_new_labels.json', 'r') as j:
            # data = j.read().decode(encoding='gbk').encode(encoding='utf-8')
            report = json.load(j)

        self.file_list = {'train':[],'val':[],'test':[]}
        self.file_list['train'] = [x["Image_path"][0] for x in report['train']]
        self.file_list['val'] = [x["Image_path"][0] for x in report['val']]
        self.file_list['test'] = [x["Image_path"][0] for x in report['test']]
        reports = report['train'] + report['val'] + report['test']
        # file_report包含所有的samples，不区分train,val,test
        self.file_report = {}
        for x in reports:
            self.file_report[x["Image_path"][0]] = x
    ...
    def preprocess(self,report):
        ...
        text = [sent_cleaner(sent) for sent in report_cleaner(report) if sent_cleaner(sent) != []]
        # Remove white space
        text = ' '.join(text) + ' .'
        return text
    ...
    def get_subsets(self, train_size=0.7, val_size=0.1, test_size=0.2, seed=0):
        ...
        train_indices = np.random.permutation(len(self.file_list['train']))
        val_indices = np.random.permutation(len(self.file_list['val']))
        test_indices = np.random.permutation(len(self.file_list['test']))

        #test
        #print(test_indices)

        train_dataset = FFAIR(self.dir, 'train', self.input_size, self.random_transform,
                               self.view_pos, self.max_views, self.sources, self.targets, self.max_len, self.vocab_file)



        # Consider change random_transform to False for validation
        val_dataset = FFAIR(self.dir, 'val', self.input_size, False,
                             self.view_pos, self.max_views, self.sources, self.targets, self.max_len, self.vocab_file)


        # Consider change random_transform to False for testing
        test_dataset = FFAIR(self.dir, 'test', self.input_size, False,
                              self.view_pos, self.max_views, self.sources, self.targets, self.max_len, self.vocab_file)
