...
from datasets import NIHCXR, MIMIC, NLMCXR, FFAIR
        ...
        for i, (source, target) in enumerate(prog_bar):
            if (i == 0) :
                ...
                if threshold != None:
                    ...
                    output = model(source[0]) #the real output funciton
...
DATASET_NAME = 'FFAIR'  # NIHCXR / NLMCXR / MIMIC/ FFAIR
BACKBONE_NAME = 'ResNet50'  # ResNeSt50 / ResNet50 / DenseNet121
MODEL_NAME = 'VisualTransformer'  # ClsGen / ClsGenInt / VisualTransformer / GumbelTransformer
...
if DATASET_NAME == 'MIMIC':
    ...
    MILESTONES = [25]  # Reduce LR by 10 after reaching milestone epochs
elif DATASET_NAME == 'FFAIR':
    EPOCHS = 70  # Start overfitting after 20 epochs
    BATCH_SIZE = 10 if PHASE == 'TRAIN' else 24  # Fit 4 GPUs
    MILESTONES = [25]  # Reduce LR by 10 after reaching milestone epochs
...
if __name__ == "__main__":
    ...
    if DATASET_NAME == 'MIMIC':
        ...
        dataset = NLMCXR('C:/XRay/Vocabulary/', INPUT_SIZE, view_pos=['AP', 'PA', 'LATERAL'], max_views=MAX_VIEWS,
        ...
        dataset = FFAIR('C:/XRay/data/FFAIR/', INPUT_SIZE, view_pos=['AP', 'PA', 'LATERAL'], max_views=MAX_VIEWS,
                         sources=SOURCES, targets=TARGETS)
        train_data, val_data, test_data = dataset.get_subsets(seed=123)
    ...
    if MODEL_NAME == 'ClsGen':
        ...
        if not RELOAD:
            checkpoint_path_from = 'C:\XRay\checkpoints/{}_ClsGen_{}_{}.pt'.format(DATASET_NAME, BACKBONE_NAME, COMMENT)
        ...
        if not RELOAD:
            checkpoint_path_from = 'C:\XRay\checkpoints/{}_Transformer_MaxView2_NumLabel{}.pt'.format(DATASET_NAME, NUM_LABELS)
        ...
        pretrained_from = 'C:\XRay\checkpoints/{}_{}_{}_{}.pt'.format(DATASET_NAME, 'VisualTransformer', BACKBONE_NAME, COMMENT)
        ...
        pretrained_from = 'C:\XRay\checkpoints/{}_{}_{}_NumLabel{}.pt'.format(DATASET_NAME, 'LSTM', 'MaxView2', NUM_LABELS)
    ...
    checkpoint_path_from = 'C:\XRay\checkpoints/{}_{}_{}_{}.pt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT)
    checkpoint_path_to = 'C:\XRay\checkpoints/{}_{}_{}_{}.pt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT)
    ...
    if PHASE == 'TRAIN':
        ...
        out_file_ref = open('C:/outputs/x_{}_{}_{}_{}_Ref.txt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT),
                            'w')
        out_file_hyp = open('C:/outputs/x_{}_{}_{}_{}_Hyp.txt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT),
                            'w')
        out_file_lbl = open('C:/outputs/x_{}_{}_{}_{}_Lbl.txt'.format(DATASET_NAME, MODEL_NAME, BACKBONE_NAME, COMMENT),
                            'w')
        print(gen_outputs)
        ...
        for i in tqdm(range(len(test_data))):
            target = test_data[i]  # caption, label
            out_file_lbl.write(' '.join(map(str, target[1])) + '\n')
        print("FFAIR输出完成，请查看outputs文件夹")
